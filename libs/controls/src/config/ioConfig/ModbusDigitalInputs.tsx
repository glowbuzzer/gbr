...
const GridRow = styled.div.attrs<GridRowProps>(({ groupIndex, ...rest }) => ({
    ...rest // Pass down all other props
}))<GridRowProps>`
    display: grid;
    grid-template-columns: 1fr 3fr 1fr 1fr 2fr 1fr 1fr 1fr 1fr;
            ...
            <StyledToolTipDiv>
                ...
                >
                    ...
                    >
                        ...
                        <Option value={2}>READ DISCRETE</Option>
                    </Select>
                </Tooltip>
            </StyledToolTipDiv>
        </GridItem>
...
// function SortButton() {
//     // State to track whether sorting is applied
//     const [isSorted, setIsSorted] = useState(false)
//
//     // Modified toggleSort function that also handles button label
//     const toggleSort = () => {
//         setIsSorted(!isSorted) // Toggle the sorted state
//     }
//
//     // Determine the label based on the isSorted state
//     const buttonLabel = isSorted ? "Unsort" : "Sort"
//
//     return <button onClick={toggleSort}>{buttonLabel}</button>
// }

/**
 * Component to manage the Modbus digital inputs
 */
export const ModbusDigitalInputs: React.FC = () => {
    const current = useModbusDigitalInputList()
    ...
    function groupConsecutiveIndices(
        indices: Set<number>,
        inputs: WithNameAndDescription<ModbusDinConfig>[]
    ): number[][] {
