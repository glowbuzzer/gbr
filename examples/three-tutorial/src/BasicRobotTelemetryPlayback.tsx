...
export type BasicRobotTelemetryPlaybackElement = {
    group: Group
    config: RobotKinematicsChainElement
}

export type BasicRobotTelemetryPlaybackProps = {
 ...
 *
 */
export const BasicRobotTelemetryPlayback = ({
    kinematicsChain,
    parts,
    jointPositions,
    ...
    scale = 1,
    children = null
}: BasicRobotTelemetryPlaybackProps) => {
    const elements = useMemo(() => {
        ...
        const elements = chain.map((element, index) => {
            ...
            return {
                group,
                config: element
            } as BasicRobotTelemetryPlaybackElement
        })
    ...
    // //
    // useFrameR3f(({ clock }) => {
    //     jointPositions[0] += Math.sin(clock.getElapsedTime()) * 0.5
    //     jointPositions[1] = 0
    //     jointPositions[2] = 0
    //     jointPositions[3] = 0
    //     jointPositions[4] = 0
    ...
    //     console.log(Math.sin(clock.getElapsedTime()) * 0.5)
    // })
