...
export function fk_delta(thetas: number[],     effectorTriangleSideLength: number,
...
function delta_calcAngleYZ( x0:number, y0:number, z0:number): {status: boolean, angle: number} {
    ...
    return (
        {status:true, angle:Math.atan(-zj/(y1 - yj)) + ((yj>y1)?Math.PI:0.0)}
    )

}


export function ik_delta(
    position: [number, number, number],
    orientation: [number, number, number, number]
): number[] {
...
var theta3 = 0;
    ...
    var result = delta_calcAngleYZ(x, y, z);

    theta1=result.angle

    if (result.status == true){
        result = delta_calcAngleYZ(x*cos120 + y*sin120, y*cos120-x*sin120, z);  // rotate coords to +120 deg
        theta2=result.angle
    }
    if (result.status == true) {
        result = delta_calcAngleYZ(x*cos120 - y*sin120, y*cos120+x*sin120, z);
        theta3=result.angle
