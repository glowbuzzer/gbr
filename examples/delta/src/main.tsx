...
const DeltaRobot = ({children = null}) => {
    ...
    const effectorVertices = [
        new THREE.Vector3(0, Math.sqrt(3) * effectorTriangleLength / 3, 0),
            new THREE.Vector3( -effectorTriangleLength / 2,-Math.sqrt(3) * effectorTriangleLength / 6, 0),
        new THREE.Vector3( effectorTriangleLength/2, -Math.sqrt(3) * effectorTriangleLength / 6, 0)
    ]
...
// const  rf = 100.0;
    ...
    useEffect(() => {
        link1Ref.current.setRotationFromAxisAngle(rotAxis1, (jointPositions[0]/1000 ||0))
        link2Ref.current.setRotationFromAxisAngle(rotAxis2, (jointPositions[1]/1000 ||0))
        link3Ref.current.setRotationFromAxisAngle(rotAxis3, (jointPositions[2]/1000 ||0))

        const fk = fk_delta([(jointPositions[0]/1000 || 0),(jointPositions[1]/1000 || 0), (jointPositions[2]/1000 || 0)], effectorTriangleLength, baseTriangleLength, upperLinkLength, lowerLinkLength )

        console.log(fk.position[0], fk.position[1], fk.position[2])
    ...
    // <group ref={robotRef}>
<>
        <TriangleExtruded vertices={baseVertices} position={[0, 0, baseZ]} color={"grey"} depth={3}/>

        <group ref={effectorRef} rotation={[0, 0, 0]}>
            <TriangleExtruded vertices={effectorVertices} position={[0, 0, 0]} color={"grey"} depth={3}/>
        </group>

        <group  ref={link1Ref} position={[baseAttachPoints[0].x,baseAttachPoints[0].y, baseZ]}>
                <Cylinder ref={ref1}  args={[upperLinkDiameter, upperLinkDiameter, upperLinkLength]}
                          position={[0, -(upperLinkLength / 2), 0]}/>
        </group>


        <group ref={link2Ref} position={[baseAttachPoints[1].x, baseAttachPoints[1].y, baseZ]} >
            <Cylinder  ref={ref2} args={[upperLinkDiameter, upperLinkDiameter, upperLinkLength]}
        ...
        <group ref={link3Ref} position={[baseAttachPoints[2].x, baseAttachPoints[2].y, baseZ]} >
            <Cylinder  ref={ref3} args={[upperLinkDiameter, upperLinkDiameter, upperLinkLength]}
                       position={[upperLinkLength*Math.sqrt(3)/4,-upperLinkLength/4 , 0]}
