...
const Laser = forwardRef<THREE.Mesh, any>(function Laser(props, ref) {
    ...
    const pointsFlat = useMemo(() => {
        const temp = []
        data.paths.forEach(path => temp.push(path.subPaths[0].getPoints()))
        return temp.flat()
    }, [data.paths])
    ...
    const vertices = useMemo(
        () =>
            pointsFlat.map(point =>
                new THREE.Vector3(
                    point.x + props.position[0],
                    point.y + props.position[1],
                    props.position[2]
                ).applyAxisAngle(axis, Math.PI)
            ),
        [pointsFlat]
    )
    ...
    spriteGeometry.setAttribute("position", new THREE.Float32BufferAttribute(justPoints, 3))
