...
const Dominos = () => {
    ...
    const colorData = Array.from({ length: number }, () => ({
    ...
    const colorArray = useMemo(
        () =>
            Float32Array.from(
                new Array(number)
                    .fill(number)
                    .flatMap((_, i) => colorTemp.set(colorData[i].color).toArray())
            ),
        []
    )
...
function BrickWall({ num }) {
    ...
    const colorArray = useMemo(
        () =>
            Float32Array.from(
                new Array(num).fill(num).flatMap((_, i) => tempColor.set(data[i].color).toArray())
            ),
        []
    )
    ...
    for (let i = 0; i < wallDims; i++) {
        for (let j = 0; j < wallDims; j++) {
            const tempPos: { x: number; y: number; z: number } = {
                x: wallCorner[0],
                y: wallCorner[1] + j * brickSpacing,
                z: wallCorner[2] + i * brickSpacing
            }
...
const PhysicsTool = props => {
    ...
    return (
        <>
            ...
            >
                <h1>Example 4 - physics engine - use the robot to knock down the dominos</h1>
            </Html>
