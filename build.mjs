...
for (const project of projects) {
        ...
        return {
            ...
            setup(build) {
                build.onResolve({ filter: /.*/ }, args => {
                    if (
                        args.kind !== "entry-point" && // this is our entry point (full path)
                        ...
                        !args.path.startsWith("@material-symbols")
                    ) {
                        // these are svgs which will be bundled by svgr
    ...
    const summary = await Promise.all(
        Object.keys(pkg.exports).map(async sub_module => {
            ...
            for (const item of imports) {
                ...
                if (!external_libs.includes(item) && !bundled_libs.includes(item)) {
                    ...
                    const candidates = Object.entries(metafile.inputs)
                        ...
                        .map(
                            ([p, info]) =>
                                `  ${p}, size ${info.bytes}, imports ${info.imports.join(", ")}`
                        )
            ...
            const import_path =
                "@glowbuzzer/" + project + (sub_module === "." ? "" : "/" + sub_module.substring(2))
            return {
                module: sub_module,
                import: import_path,
    ...
    execSync(`${tscPath} --build libs/${project}/tsconfig.lib.json`, {
        stdio: "inherit"
    })
